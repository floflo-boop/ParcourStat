...
class DataLoader:
    ...
    def load_csv_files(self, csv_dir: str):
        ...
        csv_files = [f for f in os.listdir(csv_path) if f.endswith(".csv") and not f.startswith(".~lock")] #ignore les fichiers verrous de ibre office qui se chargent automatoquement dans la base 
        ...
        with self.engine.connect() as connection:
            for csv_file in csv_files:
                ...
                with open(file_path, "r", encoding="utf-8") as f:
                    ...
                    df = pd.read_csv(
                        file_path,
                        sep=None,        # changemet ici pour créer une détection automatique du séparateur car le csv revenu_fiscal_reference est délimité par des virgules
                        engine="python", # obligatoire avec sep=None
                        encoding="utf-8-sig",
                        header=0
                    )
